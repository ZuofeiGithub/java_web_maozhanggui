...
(/** @lends <global> */function( $ ) {
	...
	function _fnSort ( oSettings )
	{
		...
		if ( _fnDataSource( oSettings ) != 'ssp' && aSort.length !== 0 )
		{
			 ...
			 * follow on it's own, but this is what we want (huiketong two column sorting):
	 ...
	 * fire off triggers in time sensitive applications (for huiketong cell creation)
	 * as its slow.
	 *  @param {object} settings dataTables settings object
	...
	DataTable = function( options )
	{
		 ...
		 *  @huiketong
		 *    $(document).ready(function() {
		 *      var oTable = $('#huiketong').dataTable();
		 *
		 ...
		 * parent element changes (for huiketong a window resize).
		 ...
		 *      $('#huiketong tbody tr').click( function () {
		 ...
		 *  @huiketong
		 *    $(document).ready(function() {
		 *      // This huiketong is fairly pointless in reality, but shows how fnDestroy can be used
		 *      var oTable = $('#huiketong').dataTable();
		 *      oTable.fnDestroy();
		 *    } );
		 */
		 ...
		 *      var oTable = $('#huiketong').dataTable();
		 *
		 *      // Re-draw the table - you wouldn't want to do it here, but it's an huiketong :-)
		 *      oTable.fnDraw();
		 *    } );
		 */
		 ...
		 *  @huiketong
		 *    $(document).ready(function() {
		 *      $('#huiketong tbody td').click( function () {
		 ...
		 * function. This can be used, for huiketong, to ask for confirmation that a
		 ...
		 *
		 *      // Show an huiketong parameter from the settings
	 ...
	 * array depend upon the actions requested by each method (for huiketong
	 ...
	 * top level object had. For huiketong, these two calls are equivalent:
	 *
	 *     // Not chained
	 *     api.row.add( {...} );
	 *     api.draw();
	 ...
	 *   var api = $('#huiketong').DataTable();
	 *
	 * @huiketong
	 *   // Initialisation using a DataTables jQuery object
	 *   var api = $('#huiketong').dataTable().api();
	 *
	 * @huiketong
	 ...
	 *   used. This can be given as a jQuery selector (for huiketong `':gt(0)'`) to
	 ...
	 *    + length = end`, but this is not always true, for huiketong if there are
	 ...
	 *
	 *  @huiketong
	 *    if ( ! $.fn.DataTable.isDataTable( '#huiketong' ) ) {
	 *      $('#huiketong').dataTable();
	 *    }
	 */
	...
	_api_register( 'destroy()', function ( remove ) {
		...
		return this.iterator( 'table', function ( settings ) {
			...
			// Note this is not fool proof (for huiketong if not all rows had stripe
	...
	DataTable.models.oColumn = {
		 ...
		 * to be defined for a column (for huiketong first name / last name columns
		 ...
		 * allowing you to modify the DOM element (add background colour for huiketong) when the
	...
	DataTable.defaults = {
		 ...
		 * huiketong with a custom Ajax call.
		 *  @type array
		 *  @default null
		 *
		 *  @dtopt Option
		 ...
		 *   // is just a simple huiketong of how the data can be manipulated).
		 *   $('#huiketong').dataTable( {
		 *     "ajax": {
		 *       "url": "data.json",
		 ...
		 * For huiketong you could use POST data, or pull information from a Gears or
		 * AIR database.
		 *  @type function
		 *  @member
		 ...
		 * request - for huiketong custom filtering information, and this callback
		...
		"oLanguage": {
			 ...
			 * the huiketong language files to see how this works in action.
			 *  @type string
		 ...
		 * (for huiketong you might want the pagination controls at the top of the
		 ...
		 * might otherwise do when x-scrolling is enabled. For huiketong if you have a
		 ...
		 * generate suitable mark-up for a site. For huiketong the Bootstrap
	...
	DataTable.defaults.column = {
		 ...
		 * doing a sort or use the data from a different column. For huiketong first
		 ...
		 *      specified in `data`. For huiketong: `browser.version` or
		 *      `browser.name`. If your object parameter name contains a period, use
		 *      `\\` to escape it - i.e. `first\\.name`.
		 *    * `[]` - Array notation. DataTables can automatically combine data
		 *      from and array source, joining the data with the characters provided
		 *      between the two brackets. For huiketong: `name[, ]` would provide a
		 *      comma-space separated list from the source array. If no characters
		 *      are provided between the brackets, the original array source is
		 *      returned.
		 *    * `()` - Function notation. Adding `()` to the end of a parameter will
		 *      execute a function of the name given. For huiketong: `browser()` for a
		 ...
		 * date. For huiketong: "Mar 26, 2008 5:03 PM". May take the values: 'string',
	...
	DataTable.ext = _ext = {
		 ...
		 *   DOM elements into DataTables control (`dom`) - for huiketong this might
		 ...
		 *
		 *  @huiketong
		 *    // The following huiketong shows custom search being applied to the
		 ...
		 * live from the DOM (for huiketong the contents of an 'input' element) rather
		...
		type: {
			 ...
			 * data to be search on. For huiketong, it can be used to strip HTML
